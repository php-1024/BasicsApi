...
use App\Http\Requests\Merchant\AliSms;
...
class SmsController extends Controller
{
    ...
    {
        $sms_url = config('app.url') . "/open/sms/ali_sms";
        /**
        ...
        if (empty($mobile)) {
            return ['code' => 50000, 'message' => '对不起检测到您的账户未设置手机号码，请您先设置手机号码'];
        }
        ...
        if (Sms::checkRowExists(['mobile' => $mobile, 'status' => 0])) {
            ...
            if (time() - $old_data['created_at'] > 180) {
                ...
                return ['code' => 50000, 'message' => '请不要重复频繁获取，请60秒后再试！'];
            }
        }
        ...
        try {
            ...
            DB::rollBack();
            return ['code' => 50000, 'message' => '获取验证码失败，请稍后再试'];
        }
        return ['code' => 20000, 'message' => '获取成功，请稍后在您的手机上查收！'];
    }
    ...
    public function get_mobile_code(Request $request)
    {
        ...
        $info = $request->get('info');
        if (empty($mobile)) {
            return ['code' => 50000, 'message' => '请您输入手机号码！'];
        }
        // 查找用户的手机号码
        $old_mobile = Account::getValue(['id' => $info['id']], 'mobile');
        if ($mobile == $old_mobile) {
            return ['code' => 50000, 'message' => '新手机号码，不能旧的手机号码相同！'];
        }
        ...
        if (Sms::checkRowExists(['mobile' => $mobile, 'status' => 0])) {
            ...
            if (time() - $old_data['created_at'] > 180) {
                ...
                return ['code' => 50000, 'message' => '请不要重频繁获取，请60秒后再试！'];
            }
        }
        DB::beginTransaction();
        try {
    ...
    {
        $dayuAccessKey = config('product.DaYuAccessKey');
        $dayuAccessSecret = config('product.DaYuAccessSecret');
        $mobile = $request->get('mobile');
        ...
        try {
            $result = AlibabaCloud::rpc()
                ...
                ->options([
                    'query' => [
                        ...
                        'PhoneNumbers' => "$mobile",
                        'SignName' => "粉丝万岁",
                        'TemplateCode' => config('product.TemplateCode'),
            ...
            $re = $result->toArray();
            if ($re['Message'] == "OK") {
                return ['code' => 20000, 'message' => '验证码发送成功'];
            } else {
                Logs::error('阿里大鱼', $result);
                $message = self::ToMessage($re);
                return ['code' => 50000, 'message' => $message];
            }
        } catch (ClientException $e) {
            return ['code' => 50000, 'message' => "错误：" . $e->getErrorMessage()];
        } catch (ServerException $e) {
            return ['code' => 50000, 'message' => "出错了：" . $e->getErrorMessage(), 'data' => [$mobile, $code, $request->all()]];
        }
    }
